...
angular.module("myApp.controllers").controller("FindCtrl", ["$scope", "$location", "AuthService", "ListService", "Pict", function($scope, $location, AuthService, ListService, Pict) {
  ...
  return console.log($scope.pictList);
}]);
...
angular.module("myApp.controllers").controller("MemberCtrl", ["$scope", "$location", "AuthService", "ListService", "AmatsukaList", function($scope, $location, AuthService, ListService, AmatsukaList) {
  ...
  $scope.onReverse = function() {
    ...
    $scope.sortText = $scope.newSortOrder ? '登録が新しい順' : '登録が古い順';
    return $scope.list.reverse();
  };
  return $scope.$watch('searchWord.screen_name', function(newData, oldData) {
    var i, idx, len, member, ref, screenNameTolowerCased;
    if (newData === oldData) {
      return;
    }
    if (newData === '') {
      ...
      ref = $scope.list.amatsukaMemberList;
...
angular.module("myApp.directives").directive('showDrawer', ["$rootScope", "TweetService", "WindowScrollableSwitcher", function($rootScope, TweetService, WindowScrollableSwitcher) {
  return {
    restrict: 'A',
    scope: {
      twitterIdStr: '@'
    ...
    link: function(scope, element, attrs) {
      ...
      return element.on('click', function() {
        var $document, body, domDrawer, domDrawerHeader, isOpenedSidebar, layer;
        ...
        isOpenedSidebar = domDrawer[0].className.indexOf('.drawer--in') !== -1;
        if (isOpenedSidebar) {
          showDrawer();
          return;
        }
...
MaoContainerController = (function() {
  ...
  MaoContainerController.prototype.fetchTabData = function() {
    var targetDates, tasks;
    targetDates = this.TimeService.getDatesPerMonth();
    tasks = targetDates.map((function(_this) {
      return function(date) {
        var qs;
        qs = _this.$httpParamSerializer({
          date: date
        });
        return _this.MaoService.countTweetByMaoTokenAndDate(qs);
      };
    })(this));
    return Promise.all(tasks).then((function(_this) {
      return function(responses) {
        console.log(responses);
        _this.tabs = responses.map(function(response, i) {
          return {
  ...
  MaoContainerController.prototype.activateLink = function(clickedTab) {
    this.tabType = clickedTab.id;
    return this.tabs.map(function(tab) {
      return tab.active = tab.id === clickedTab.id ? true : false;
    });
  };

  MaoContainerController.prototype.onSelected = function(clickedTab) {
    this.activateLink(clickedTab);
    return this.$scope.$broadcast('MaoContainerController::paginate', {
      date: clickedTab.id
    });
  };

  MaoContainerController.prototype.subscribe = function() {
    return this.$scope.$on('termPagination::paginate', (function(_this) {
      return function(event, args) {
        return _this.activateLink({
          id: args.date
        });
      };
    })(this));
...
MaoListContoller = (function() {
  function MaoListContoller($location, $httpParamSerializer, $scope, Mao, MaoService, URLParameterChecker, TimeService) {
    ...
    this.Mao = Mao;
    this.MaoService = MaoService;
    this.URLParameterChecker = URLParameterChecker;
    this.TimeService = TimeService;
    this.date = null;
    this.tweetTotalNumber = 0;
    this.setDate();
    this.tweetList = new this.Mao(this.date);
    this.getTweetTotalNumber();
    this.subscribe();
  }

  MaoListContoller.prototype.setDate = function() {
    var urlParameterChecker;
    urlParameterChecker = new this.URLParameterChecker();
    if (_.isEmpty(urlParameterChecker.queryParams)) {
      urlParameterChecker.queryParams.date = moment().add(-1, 'days').format('YYYY-MM-DD');
    }
    return this.date = moment(urlParameterChecker.queryParams.date).format('YYYY-MM-DD');
  };

  MaoListContoller.prototype.getTweetTotalNumber = function() {
    var qs;
    qs = this.$httpParamSerializer({
      date: this.date
    });
    return this.MaoService.countTweetByMaoTokenAndDate(qs).then((function(_this) {
  ...
  MaoListContoller.prototype.subscribe = function() {
    this.$scope.$on('MaoStatusController::publish', (function(_this) {
      return function(event, args) {
        _this.date = args.date;
      };
    })(this));
    this.$scope.$on('termPagination::paginate', (function(_this) {
      return function(event, args) {
    ...
    return this.$scope.$on('MaoContainerController::paginate', (function(_this) {
      return function(event, args) {
