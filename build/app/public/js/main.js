angular.module('myApp', [
  ...
  function ($routeProvider, $locationProvider) {
    ...
    }).when('/logout', { redirectTo: '/' }).when('http://127.0.0.1:4321/auth/twitter/callback', { redirectTo: '/' });
...
angular.module('myApp.controllers', []).controller('CommonCtrl', [
  '$location',
  '$log',
  '$rootScope',
  '$scope',
  function ($location, $log, $rootScope, $scope) {
    return $rootScope.$on('$locationChangeStart', function (event, next, current) {
      $log.info('location changin to: ' + next);
    });
  }
]);
angular.module('myApp.directives', []).directive('boxLoading', [
  '$interval',
  function ($interval) {
    return {
      restrict: 'E',
  ...
  return {
    ...
    link: function (scope, element, attrs) {
      return element.on('click', function () {
        return $('html, body').animate({ scrollTop: $(scope.scrollTo).offset().top }, 'slow');
      });
    }
  };
});
...
angular.module('myApp.controllers').controller('AdminUserCtrl', [
  '$scope',
  '$rootScope',
  '$log',
  'AuthService',
...
angular.module('myApp.controllers').controller('IndexCtrl', [
  '$scope',
  '$log',
  'AuthService',
  'TweetService',
  function ($scope, $log, AuthService, TweetService) {
...
angular.module('myApp.services').service('AuthService', [
  '$http',
  function ($http) {
    return {
...
angular.module('myApp.services').service('TweetService', [
  '$http',
  function ($http) {
    return {
